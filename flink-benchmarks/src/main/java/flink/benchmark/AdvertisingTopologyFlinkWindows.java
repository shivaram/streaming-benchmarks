...
import flink.benchmark.EventGeneratorSource;
...
public class AdvertisingTopologyFlinkWindows {
  ...
  public static void main(final String[] args) throws Exception {
    ...
    EventGeneratorSource eventGenerator = new EventGeneratorSource(
        loadTargetHz, timeSliceLengthMs, totalNumElementsToGenerate);
    RichParallelSourceFunction<String> source = eventGenerator;
    String sourceName = "EventGenerator";
    Map<String, List<String>> campaigns = eventGenerator.getCampaigns();
    Map<String, String> adsToCampaignsMap = new HashMap<String, String>();
    for (Map.Entry<String, List<String>> entry : campaigns.entrySet()) {
      for (String ad : entry.getValue()) {
        adsToCampaignsMap.put(ad, entry.getKey());
      }
    }
    ...
    DataStream<Tuple2<String, String>> joinedAdImpressions = rawMessageStream
      ...
      .flatMap(new JoinBolt(adsToCampaignsMap)) // campaign_id, event_time
    ...
    WindowedStream<Tuple3<String, String, Long>, Tuple, TimeWindow> windowStream = joinedAdImpressions
      ...
      .keyBy(0) // campaign_id
      .timeWindow(Time.of(configWindowSize, TimeUnit.MILLISECONDS));
    ...
    // result.addSink();
    // if (useRedis) {
    //  result.addSink(new RedisResultSink(redisHost));
    // } else {
    result.writeAsCsv(csvPath, FileSystem.WriteMode.OVERWRITE);
    // }

    env.execute("AdvertisingTopologyFlinkWindows " + parameter.toMap().toString());
  }
  ...
  private static final class JoinBolt extends RichFlatMapFunction<Tuple2<String, String>, Tuple2<String, String>> {
    ...
    public void flatMap(Tuple2<String, String> input, Collector<Tuple2<String, String>> out) throws Exception {
      ...
      String campaign_id = adsToCampaignsMap.get(ad_id);
      if (campaign_id == null) {
        return;
      }
  ...
  private static class RedisResultSink extends RichSinkFunction<Tuple3<String, String, Long>> {
    ...
    public void open(Configuration parameters) throws Exception {
      ...
      flushJedis = new Jedis(redisHost);
    }
